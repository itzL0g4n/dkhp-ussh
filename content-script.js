// Content Script - Intercept network requests and log them
// This script can be toggled from the extension popup

(function() {
    // Check if interception is enabled
    chrome.storage.sync.get(['enableInterception'], (result) => {
        if (result.enableInterception) {
    ...
    function initializeInterceptors() {
        console.clear();
        console.log("%c🔥 ĐÃ KÍCH HOẠT CHẾ ĐỘ BẮT TẤT CẢ GÓI TIN", "color: white; background: #d32f2f; font-size: 18px; font-weight: bold; padding: 5px 10px;");
        ...
        window.fetch = async (...args) => {
            ...
            if (url.includes('hcmussh.edu.vn') && !url.includes('.js') && !url.includes('.css')) {
                console.group(`%c📡 FETCH: ${url}`, "color: #1976d2; font-weight: bold;");
                
                if (config && config.body) {
                    const parsed = parseBody(config.body);
                    console.log("%c📤 PAYLOAD:", "color: #388e3c; font-weight: bold;", parsed);
        ...
        XMLHttpRequest.prototype.send = function(body) {
            if (this._url.includes('api') || this._url.includes('hoc-phan') || this._url.includes('dang-ky')) {
                console.group(`%c📨 XHR: ${this._url}`, "color: #1976d2; font-weight: bold;");
